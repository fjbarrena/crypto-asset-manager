...
Then('Wait for {int} milliseconds', {timeout: 60 * 1000}, async function (milliseconds) {
...
Then('Save the response property {string} in the key {string}', function (propertyName, key) {
    const value = TestContext.resultDataInUse;
...
When('calls with {string} {string}', async function (httpVerb: string, endpoint: string) {
...
Then('Check that response property {string} is {int}', function (property, expectedValue) {
    const receivedValue = TestContext.resultDataInUse[property];
    expect(receivedValue).toEqual(expectedValue);
});

Then('Check that response property {string} is {string}', function (property, expectedValue) {
    const receivedValue = TestContext.resultDataInUse[property];
    expect(receivedValue).toEqual(expectedValue);
});
...
Then('Check that response property {string} is boolean {boolean}', function (property, expectedValue) {
    console.log(TestContext.resultDataInUse)
    const receivedValue = TestContext.resultDataInUse[property];
    expect(receivedValue).toEqual(expectedValue);
});

Then('Check that response array at {string} contains at least one item with the property {string} with the value {string}', function (arrayProperty, containsProperty, expectedValue) {  
